...
class Js_Dbconn {
    ...
    {
        //Delete clauses require a where param, otherwise use "trunpayslipe"
        if( empty( $where ) )
        {
            return false;
            exit;
    ...
    public function trunpayslipe( $tables = array() )
    {
        if( !empty( $tables ) )
        {
            $trunpaysliped = 0;
            ...
            {
                $trunpayslipe = "TRUNCATE TABLE `".trim($table)."`";
                $this->link->query( $trunpayslipe );
                if( !$this->link->error )
                {
                    $trunpaysliped++;
                }
            }
            return $trunpaysliped;
        }
    }
